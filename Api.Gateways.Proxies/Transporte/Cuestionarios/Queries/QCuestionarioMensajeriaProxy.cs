...
namespace Api.Gateway.Proxies.Transporte.Cuestionarios.Queries
{
    public interface IQCuestionarioTransporteProxy
    {
    ...
    public class QCuestionarioTransporteProxy : IQCuestionarioTransporteProxy
    {
        private readonly ApiUrls _apiUrls;
        private readonly HttpClient _httpClient;

        public QCuestionarioTransporteProxy(HttpClient httpClient, IOptions<ApiUrls> apiUrls, IHttpContextAccessor httpContextAccessor)
        {
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiUrls.TransporteUrl}api/transporte/cuestionarios");
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiUrls.TransporteUrl}api/transporte/cuestionarios/{anio}/{mes}/{contrato}");
            request.EnsureSuccessStatusCode();
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiUrls.TransporteUrl}api/transporte/cuestionarios/getPreguntasConDeductiva/{anio}/{mes}/{contrato}");
            request.EnsureSuccessStatusCode();
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiUrls.TransporteUrl}api/transporte/cuestionarios/getPreguntaById/{pregunta}");
            request.EnsureSuccessStatusCode();
