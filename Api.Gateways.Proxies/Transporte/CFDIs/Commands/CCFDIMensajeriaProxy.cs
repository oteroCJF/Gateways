...
namespace Api.Gateway.Proxies.Transporte.CFDIs.Commands
{
    public interface ICCFDITransporteProxy
    {
    ...
    public class CCFDITransporteProxy : ICCFDITransporteProxy
    {
        private readonly ApiUrls _apiUrls;
        private readonly HttpClient _httpClient;

        public CCFDITransporteProxy(HttpClient httpClient, IOptions<ApiUrls> apiUrls, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<CFDIDto> CreateFactura([FromForm] CFDICreateCommand factura)
        {
            ...
            var request = await _httpClient.PostAsync($"{_apiUrls.TransporteUrl}api/transporte/cfdi/createFactura", formContent);
        ...
        public async Task<CFDIDto> UpdateFactura([FromForm] CFDIUpdateCommand command)
        {
            ...
            var request = await _httpClient.PutAsync($"{_apiUrls.TransporteUrl}api/transporte/cfdi/updateFactura", formContent);
            request.EnsureSuccessStatusCode();
        ...
        public async Task CreateHistorialMF([FromBody] HistorialMFCreateCommand command)
        {
            ...
            var request = await _httpClient.PostAsync($"{_apiUrls.TransporteUrl}api/transporte/cfdi/createHistorialMF", content);
            request.EnsureSuccessStatusCode();
