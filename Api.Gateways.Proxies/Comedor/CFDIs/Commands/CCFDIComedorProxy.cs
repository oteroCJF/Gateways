...
namespace Api.Gateway.Proxies.Comedor.CFDIs.Commands
{
    public interface ICCFDIComedorProxy
    {
    ...
    public class CCFDIComedorProxy : ICCFDIComedorProxy
    {
        private readonly ApiUrls _apiUrls;
        private readonly HttpClient _httpClient;

        public CCFDIComedorProxy(HttpClient httpClient, IOptions<ApiUrls> apiUrls, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<CFDIDto> CreateFactura([FromForm] CFDICreateCommand factura)
        {
            ...
            var request = await _httpClient.PostAsync($"{_apiUrls.ComedorUrl}api/comedor/cfdi/createFactura", formContent);
        ...
        public async Task<CFDIDto> UpdateFactura([FromForm] CFDIUpdateCommand command)
        {
            ...
            var request = await _httpClient.PutAsync($"{_apiUrls.ComedorUrl}api/comedor/cfdi/updateFactura", formContent);
            request.EnsureSuccessStatusCode();
        ...
        public async Task CreateHistorialMF([FromBody] HistorialMFCreateCommand command)
        {
            ...
            var request = await _httpClient.PostAsync($"{_apiUrls.ComedorUrl}api/comedor/cfdi/createHistorialMF", content);
            request.EnsureSuccessStatusCode();
