...
namespace Api.Gateway.Proxies.Fumigacion.Historiales
{
    ...
    public class FLEntregableProxy : IFLEntregableProxy
    {
        ...
        public FLEntregableProxy(HttpClient httpClient, IOptions<ApiUrls> apiUrls, IHttpContextAccessor httpContextAccessor)
        {
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiUrls.FumigacionUrl}api/fumigacion/logEntregables/getHistorialEntregablesByCedula/{cedula}");
        ...
        public async Task CreateHistorial([FromBody] LogEntregableCreateCommand historial)
        {
            ...
            var request = await _httpClient.PostAsync($"{_apiUrls.FumigacionUrl}api/fumigacion/logEntregables/createHistorial", content);
            request.EnsureSuccessStatusCode();
        }
    }
}
