...
namespace Api.Gateway.Proxies.Mensajeria.CFDIs.Commands
{
    ...
    public class CCFDIMensajeriaProxy : ICCFDIMensajeriaProxy
    {
        ...
        public CCFDIMensajeriaProxy(HttpClient httpClient, IOptions<ApiUrls> apiUrls, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<CFDIDto> CreateFactura([FromForm] CFDICreateCommand factura)
        {
            ...
            var request = await _httpClient.PostAsync($"{_apiUrls.MensajeriaUrl}api/mensajeria/cfdi/createFactura", formContent);
        ...
        public async Task<CFDIDto> UpdateFactura([FromForm] CFDIUpdateCommand command)
        {
            ...
            var request = await _httpClient.PutAsync($"{_apiUrls.MensajeriaUrl}api/mensajeria/cfdi/updateFactura", formContent);
            request.EnsureSuccessStatusCode();
        ...
        public async Task CreateHistorialMF([FromBody] HistorialMFCreateCommand command)
        {
            ...
            var request = await _httpClient.PostAsync($"{_apiUrls.MensajeriaUrl}api/mensajeria/cfdi/createHistorialMF", content);
            request.EnsureSuccessStatusCode();
