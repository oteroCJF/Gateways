...
namespace Api.Gateway.Proxies.Agua.Firmantes.Queries
{
    ...
    public class QFirmanteAguaProxy : IQFirmanteAguaProxy
    {
        ...
        public QFirmanteAguaProxy(HttpClient httpClient, IOptions<ApiUrls> apiUrls, IHttpContextAccessor httpContextAccessor)
        {
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiUrls.AguaUrl}api/agua/firmantes");
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiUrls.AguaUrl}api/agua/firmantes/getFirmanteById/{firmante}");
            request.EnsureSuccessStatusCode();
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiUrls.AguaUrl}api/agua/firmantes/getFirmantesByInmueble/{inmueble}");
            request.EnsureSuccessStatusCode();
