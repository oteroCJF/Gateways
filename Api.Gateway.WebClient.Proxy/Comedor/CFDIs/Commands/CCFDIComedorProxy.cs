...
namespace Api.Gateway.WebClient.Proxy.Comedor.CFDIs.Commands
{
    ...
    public class CCFDIComedorProxy : ICCFDIComedorProxy
    {
        ...
        public CCFDIComedorProxy(HttpClient httpClient, ApiGatewayUrl apiGatewayUrl, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<CFDIDto> CreateFactura([FromForm] CFDICreateCommand factura)
        {
            ...
            var request = await _httpClient.PostAsync($"{_apiGatewayUrl}comedor/cfdi/createFactura", formContent);
        ...
        public async Task<CFDIDto> UpdateFactura([FromForm] CFDIUpdateCommand factura)
        {
            ...
            var request = await _httpClient.PutAsync($"{_apiGatewayUrl}comedor/cfdi/updateFactura", formContent);
            request.EnsureSuccessStatusCode();
        ...
        public async Task CreateHistorialMF([FromBody] HistorialMFCreateCommand command)
        {
            ...
            var request = await _httpClient.PostAsync($"{_apiGatewayUrl}comedor/cfdi/createHistorialMF", content);
            request.EnsureSuccessStatusCode();
