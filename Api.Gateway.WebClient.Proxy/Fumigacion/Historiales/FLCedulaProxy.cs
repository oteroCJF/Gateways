...
namespace Api.Gateway.WebClient.Proxy.Fumigacion.Historiales
{
    ...
    public class FLCedulaProxy : IFLCedulaProxy
    {
        ...
        public FLCedulaProxy(HttpClient httpClient, ApiGatewayUrl apiGatewayUrl, IHttpContextAccessor httpContextAccessor)
        {
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiGatewayUrl}fumigacion/logCedulas/getHistorialByCedula/{cedula}");
        ...
        public async Task CreateHistorial(LogCedulaCreateCommand historial)
        {
            ...
            var request = await _httpClient.PostAsync($"{_apiGatewayUrl}fumigacion/logCedulas/createHistorial", content);
            request.EnsureSuccessStatusCode();
        }
    }
}
