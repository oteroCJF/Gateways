...
namespace Api.Gateway.WebClient.Proxy.Agua.CFDIs.Commands
{
    ...
    public class CCFDIAguaProxy : ICCFDIAguaProxy
    {
        ...
        public CCFDIAguaProxy(HttpClient httpClient, ApiGatewayUrl apiGatewayUrl, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<CFDIDto> CreateFactura([FromForm] CFDICreateCommand factura)
        {
            ...
            var request = await _httpClient.PostAsync($"{_apiGatewayUrl}agua/cfdi/createFactura", formContent);
        ...
        public async Task<CFDIDto> UpdateFactura([FromForm] CFDIUpdateCommand factura)
        {
            ...
            var request = await _httpClient.PutAsync($"{_apiGatewayUrl}agua/cfdi/updateFactura", formContent);
            request.EnsureSuccessStatusCode();
        ...
        public async Task CreateHistorialMF([FromBody] HistorialMFCreateCommand command)
        {
            ...
            var request = await _httpClient.PostAsync($"{_apiGatewayUrl}agua/cfdi/createHistorialMF", content);
            request.EnsureSuccessStatusCode();
