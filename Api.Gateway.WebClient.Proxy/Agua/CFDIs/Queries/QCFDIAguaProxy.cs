...
namespace Api.Gateway.WebClient.Proxy.Agua.CFDIs.Queries
{
    ...
    public class QCFDIAguaProxy : IQCFDIAguaProxy
    {
        ...
        public QCFDIAguaProxy(HttpClient httpClient, ApiGatewayUrl apiGatewayUrl, IHttpContextAccessor httpContextAccessor)
        {
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiGatewayUrl}agua/cfdi/getAllFacturas");
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiGatewayUrl}agua/cfdi/getFacturasByRepositorio/{repositorio}");
            request.EnsureSuccessStatusCode();
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiGatewayUrl}agua/cfdi/getFacturasByInmueble/{inmueble}/{facturacion}");
            request.EnsureSuccessStatusCode();
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiGatewayUrl}agua/cfdi/getHistorialMFByFacturacion/{facturacion}");
            request.EnsureSuccessStatusCode();
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiGatewayUrl}agua/cfdi/visualizarFactura/{anio}/{mes}/{folio}/{tipo}/{inmueble}/{archivo}");
            request.EnsureSuccessStatusCode();
