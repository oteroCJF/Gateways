...
namespace Api.Gateway.WebClient.Proxy.Agua.Repositorios.Commands
{
    ...
    public class QRepositorioAguaProxy : IQRepositorioAguaProxy
    {
        ...
        public QRepositorioAguaProxy(HttpClient httpClient, ApiGatewayUrl apiGatewayUrl, IHttpContextAccessor httpContextAccessor)
        {
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiGatewayUrl}agua/repositorios/{anio}");
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiGatewayUrl}agua/repositorios/getRepositorioByAMC/{anio}/{mes}/{contrato}");
            request.EnsureSuccessStatusCode();
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiGatewayUrl}agua/repositorios/getRepositorioById/{facturacion}");
            request.EnsureSuccessStatusCode();
