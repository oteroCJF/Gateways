...
namespace Api.Gateway.WebClient.Proxy.Limpieza.Entregables
{
    ...
    public class LEntregableProxy : ILEntregableProxy
    {
        ...
        public LEntregableProxy(HttpClient httpClient, ApiGatewayUrl apiGatewayUrl, IHttpContextAccessor httpContextAccessor)
        {
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiGatewayUrl}limpiezalimpieza/entregablesCedula/getEntregablesByEstatus/{estatus}");
        ...
        {
            var request = await _httpClient.GetAsync($"{_apiGatewayUrl}limpieza/entregablesCedula/getEntregablesByCedula/{cedula}");
            request.EnsureSuccessStatusCode();
        ...
        public async Task UpdateEntregable(EntregableCommandUpdate entregable)
        {
            ...
            var request = await _httpClient.PutAsync($"{_apiGatewayUrl}limpieza/entregablesCedula/actualizarEntregable", formContent);
            request.EnsureSuccessStatusCode();
        }
        ...
        public async Task AUpdateEntregable([FromForm] EEntregableUpdateCommand entregable)
        {
            ...
            var request = await _httpClient.PutAsync($"{_apiGatewayUrl}limpieza/entregablesCedula/AREntregable", formContent);
            request.EnsureSuccessStatusCode();
        }

        public async Task<string> VisualizarEntregables(int anio, string mes, string folio, string archivo, string tipo)
        {
            var request = await _httpClient.GetAsync($"{_apiGatewayUrl}limpieza/entregablesCedula/visualizarEntregable/{anio}/{mes}/{folio}/{archivo}/{tipo}");
            request.EnsureSuccessStatusCode();
        ...
        public async Task<string> DescargarEntregables([FromBody] DEntregablesCommand command)
        {
            ...
            var request = await _httpClient.PostAsync($"{_apiGatewayUrl}limpieza/entregablesCedula/descargarEntregables", content);
            request.EnsureSuccessStatusCode();
