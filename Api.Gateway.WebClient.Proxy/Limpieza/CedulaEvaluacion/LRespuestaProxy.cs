...
namespace Api.Gateway.WebClient.Proxy.Limpieza.CedulaEvaluacion
{
    ...
    public class LRespuestaProxy : ILRespuestaProxy
    {
        ...
        public LRespuestaProxy(HttpClient httpClient, ApiGatewayUrl apiGatewayUrl, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task UpdateRespuestas(List<RespuestasUpdateCommand> respuestas)
        {
            ...
            var request = await _httpClient.PutAsync($"{_apiGatewayUrl}limpieza/cedulaEvaluacion/updateRespuestasByCedula", content);
